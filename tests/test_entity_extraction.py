...
def test_entity_extraction():
    ...
    for entity_type, entities in result["entities"].items():
        for entity in entities:
            ...
            print(
                f"{entity_type}: '{entity['text']}' at [{entity['start']}:{entity['end']}]"
            )
            print(
                f"  Verification: text[{entity['start']}:{entity['end']}] = '{extracted_text}'"
            )
...
def test_batch_entity_extraction():
    ...
    results = model.batch_extract_entities(
        texts, entity_types, batch_size=2, include_confidence=True, include_spans=True
    )
